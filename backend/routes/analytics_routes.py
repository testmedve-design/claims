...
def get_hospital_analytics():
    ...
    try:
        ...
        for claim in claims:
            ...
            amount = float(claim.get('form_data', {}).get('claimed_amount', 0) or 0)
            stats['total_amount'] += amount
            
            # Approximate approved amount logic (real logic depends on approved_amount field availability)
            if status in ['approved', 'settled', 'claim_approved', 'qc_clear']: 
                # Use approved_amount if available, else claimed amount for estimate
                app_amt = float(claim.get('approved_amount', 0) or amount) 
            ...
            # Payer Performance
            payer = claim.get('form_data', {}).get('payer_name', 'Unknown')
            ...
            stats['payer_performance'][payer]['amount'] += amount
            if status in ['approved', 'settled', 'claim_approved']:
...
def get_review_analytics():
    ...
    try:
        ...
        for claim in claims:
            ...
            review_status = claim.get('review_status', 'pending') # Default to pending if in this bucket but not set
            ...
            if review_status == 'reviewed':
                ...
                stats['financials']['claimed'] += float(review_data.get('claimed_amount', 0) or 0)
                stats['financials']['approved'] += float(review_data.get('approved_amount', 0) or 0)
                stats['financials']['disallowed'] += float(review_data.get('disallowed_amount', 0) or 0)
...
def get_rm_analytics():
    ...
    try:
        ...
        for claim in claims:
            ...
            c_payer = claim.get('form_data', {}).get('payer_name', '').upper()
            
            # Hospital Assignment Check
            if assigned_hospitals:
                if c_hosp_id not in [h['id'] for h in assigned_hospitals]:
                    continue
            
            # Payer Assignment Check
            if assigned_payers:
                assigned_names = [p['name'].upper() for p in assigned_payers]
                # Simple substring match or exact? Let's do partial match
                if not any(p_name in c_payer for p_name in assigned_names):
                    continue
        ...
        for claim in claims:
            status = claim.get('claim_status')
            rm_status = claim.get('rm_status') or status # Fallback
            
            if rm_status in ['settled', 'partially_settled']:
                stats['settled_claims'] += 1
                
                # Financials from RM data
                rm_data = claim.get('rm_data', {})
                stats['financials']['settled'] += float(rm_data.get('settled_amount', 0) or 0)
